...
export default class Headlines extends React.Component {
  constructor(props) {
    ...
    this.toggleSearchMenuOnBodyClick = this.toggleSearchMenuOnBodyClick.bind(
      this,
    );
  ...
  toggleSidebar(e) {
    ...
    if (this.state.isSidebarOpen) {
      ...
      if (
        !this.sameSources(
          this.state.passedSources.split(','),
          this.state.storedSources,
        )
      ) {
  ...
  sameSources(oldSource, currentSource) {
    ...
    if (oldSource.length !== currentSource.length) {
      ...
      // else they are the same length, check content

      for (let i = 0; i < currentSource.length; i++) {
        if (oldSource.includes(currentSource[i])) {
          // if found, check next item
          continue;
        }
